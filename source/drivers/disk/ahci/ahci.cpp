...
namespace kernel::drivers::disk::ahci {
...
bool AHCIPort::read(uint64_t sector, uint32_t sectorCount, uint16_t *buffer)
{
    return rw(sector, sectorCount, buffer, false);
}

bool AHCIPort::write(uint64_t sector, uint32_t sectorCount, uint16_t *buffer)
{
