...
void RTL8169::send(uint8_t *data, uint64_t length)
{
...
void RTL8169::receive()
{
    for (size_t i = 0; (inb(this->IOBase + 0x37) & 0x01) == 0; i++)
    {
        ...
        size_t length = (this->rxdescs[this->rxcurr]->command & 0x00001FFF) - 4;
...
void RTL8169::rxinit()
{
    Desc *descs = static_cast<Desc*>(malloc(sizeof(Desc) * RTL8169_NUM_RX_DESC + 16));
    for (size_t i = 0; i < RTL8169_NUM_RX_DESC; i++)
    {
        this->rxdescs[i] = reinterpret_cast<Desc*>(reinterpret_cast<uint8_t*>(descs) + i * 16);
        if (i == RTL8169_NUM_RX_DESC) this->rxdescs[i]->command = RTL8169_OWN | RTL8169_EOR | (RTL8169_RX_BUFF_SIZE & 0x3FFF);
...
void RTL8169::txinit()
{
    Desc *descs = static_cast<Desc*>(malloc(sizeof(Desc) * RTL8169_NUM_TX_DESC + 16));
    for (size_t i = 0; i < RTL8169_NUM_TX_DESC; i++)
    {
        this->txdescs[i] = reinterpret_cast<Desc*>(reinterpret_cast<uint8_t*>(descs) + i * 16);
        if (i == RTL8169_NUM_TX_DESC) this->rxdescs[i]->command = RTL8169_EOR;
    }
}
